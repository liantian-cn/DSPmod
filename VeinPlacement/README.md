...
只有在极端情况下实在找不到满足组内间距的点时，才会记录 warning，并以“保留矿点数量”为优先做放松 fallback。
...
- 插件不再基于水陆或海拔过滤矿组中心；水下、山上都允许。
- 组内矿点在重排后贴地形高度，可能低于海平面；插件不会重新执行原版“水下则不加入矿池”的删除逻辑，因为目标是保持原有数量。
...
5. 在赤道随机选择一个中心经度
6. 在纬度 `±40°`、经度 `±90°` 的窗口内重排矿组中心，不按水陆或海拔过滤
7. 在每个矿组中心周围重新生成组内矿点偏移
8. 写回 `VeinData.pos`
9. 对油矿重新执行 `RawSnap`
10. 用 `QueryHeight()` 贴回该方向的地形高度，可能低于海平面
11. 清理新位置植被
